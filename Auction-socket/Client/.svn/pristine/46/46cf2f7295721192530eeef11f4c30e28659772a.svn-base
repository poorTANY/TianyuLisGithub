...
public class WaitingRoomView extends CustomPanel implements Observer {
	...
	/**
	 * @author jep537 WaitingRoomVierw creates a JPanel which contains all the
	 *         necessary features for the user to log in. The class uses custom
	 *         components created for the GUI and sets up a method for automatic
	 *         updates of the room table every 15 seconds.
	 */
	 ...
	 * 
	 * @param model
	 *            WaitingRoom model for this object
	 * @param table
	 *            Table model for the room table
	 * @param frame
	 *            Frame for this class to be shown in
	 */
	public WaitingRoomView(WaitingRoom model, JTable table, JFrame frame) {
		...
		button_3.addActionListener(ae -> {
			if (table.getSelectedRow() != -1) {
				model.selectRoom(table.getValueAt(table.getSelectedRow(), 1).toString());
			} else {
				JOptionPane.showMessageDialog(frame, "Please select a room", "Error", JOptionPane.WARNING_MESSAGE);
			}
		});
		...
		confirmButton.addActionListener(ae -> {
			if (creation == true) {
				if (!roomName.getText().equals("")) {
					model.addRoom(roomName.getText(), (String) availability.getSelectedItem(),
							(Integer) numberOfPlayers.getSelectedItem());
				...
				if (table.getSelectedRow() != -1) {
					model.editRoom(table.getValueAt(table.getSelectedRow(), 1).toString(),
							table.getValueAt(table.getSelectedRow(), 4).toString(),
							availability.getSelectedItem().toString(), numberOfPlayers.getSelectedItem().toString());
					hideRoomEditOptions();
				}
			}
		});
	...
	/**
	 * Shows all buttons necessary for creation of rooms and hides the other
	 * buttons.
	 */
	...
	/**
	 * Hides all buttons necessary for creation of rooms and shows the other
	 * buttons.
	 */
	...
	/**
	 * Shows all buttons necessary for editing rooms and hides the other
	 * buttons.
	 */
	...
	/**
	 * Hides all buttons necessary for editing rooms and shows the other
	 * buttons.
	 */
